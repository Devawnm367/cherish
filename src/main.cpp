...
#if defined(NDEBUG)
# error "Cherishcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2f4449743f712b74f28da75cffd36d9b1b0e2676ed0272c7c57360b40a1ae5d9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cherishcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cherishcoin Signed Message:\n";
...
{
    // Cherishcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 21000); // Cherishcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Cherishcoin: 3.5 days
static const int64 nTargetSpacing = 7 * 60; // Cherishcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cherishcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cherishcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x239d452724d0bbfc1ededdb53cb50245d5e392a8581d25053060394e8915d09f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Today a breakthrough in research and healthcar was 01/05\19 created ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 48 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044e7dcc8099a7f5ba90c3e7f76e0af0f490fc5fba8b9977f82e1cb59b734274b45594e07617130bc9e9d36f481f9cf17e0bb85d72ee8cbae46718eaaeefe25c84") << OP_CHECKSIG;
        CBlock block;
        ...
        block.nNonce   = 2085205057;

        if (fTestNet)
        {
            block.nTime    = 1554515595;
            block.nNonce   = 1555900022;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7227c7f2cc2b3feccf3a005158a19e95ac7a60440585901bfa7d30de26c89ae1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfu, 0xc4, 0xb4, 0xdc }; // Cherishcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CherishcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CherishcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CherishcoinMiner(CWallet *pwallet)
{
    printf("CherishcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cherishcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CherishcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CherishcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CherishcoinMiner, pwallet));
}
